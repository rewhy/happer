...
def FindClass():
	...
	soa_param = int(execution_state.getRegisterService().getValue("R1")) & 0xffffffff
	...
	if config.package_name.replace(".", "/") in descriptor_string_val:
		mirror_class_names, mirror_class_ptrs = force_loading(descriptor_string_val)
		config.save_class_info(thread_ptr, mirror_class_names, mirror_class_ptrs)
...
def force_loading(exclude_class):
	# for storing loaded mirror::Class pointer
	mirror_class_names = []
	mirror_class_ptrs = []
	...
	for class_def_idx in range(class_defs_size):
		...
		# retrieve loaded mirror::Class
		class_ptr = int(execution_state.getRegisterService().getValue("R0")) & 0xffffffff
		mirror_class_ptrs.append(class_ptr)
		mirror_class_names.append(class_descriptor_content)
	...
	class_ptr = int(execution_state.getRegisterService().getValue("R0")) & 0xffffffff
	mirror_class_ptrs.append(class_ptr)
	mirror_class_names.append(exclude_class)
	
	return mirror_class_names, mirror_class_ptrs
