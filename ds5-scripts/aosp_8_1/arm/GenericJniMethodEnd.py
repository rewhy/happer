# GenericJniMethodEnd.py is used to ... when a jni method is invoked in 32-bit mode.
...
def dump_libart_memory():
	file_path = os.path.join(config.workspace, config.libart_memory_directory, ("%s_%s_%s.bin" % ("libart", "GenericJniMethodEnd", str(time.time()))))
	file_format = "binary"
...
def dump_libc_memory():
	file_path = os.path.join(config.workspace, config.libc_memory_directory, ("%s_%s_%s.bin" % ("libc", "GenericJniMethodEnd", str(time.time()))))
	file_format = "binary"
...
def GenericJniMethodEnd():
	config.log_print("[GenericJniMethodEnd] ...")
	...
	# if config.debug:
		# print "[GenericJniMethodEnd] ArtMethod::declaring_class_::dex_cache_::dex_file_::begin_ = %0#10x" % dex_file_begin_val
	...
	# else:
		...
		# if config.debug:
			# print "[GenericJniMethodEnd] ArtMethod::declaring_class_::dex_cache_::dex_file_::size_ = %#x" % dex_file_size_val
		...
		# if config.debug:
			# print "[GenericJniMethodEnd] ArtMethod::declaring_class_::dex_cache_::dex_file_::location_ = %s" % dex_file_location_string_val
		...
		# if config.debug:
			# print "[GenericJniMethodEnd] ArtMethod::access_flags_ = %#x (%s)" % (art_method_access_flags_value, config.resolve_access_flags(art_method_access_flags_value))
		...
		# if config.debug:
			# print "[GenericJniMethodEnd] ArtMethod::dex_code_item_offset_ = %#x" % art_method_dex_code_item_offset_value
		...
		# if config.debug:
			# print "[GenericJniMethodEnd] ArtMethod::dex_method_index_ = %#x" % art_method_dex_method_index_val
		...
		# # if config.debug:
			# # print "[GenericJniMethodEnd] class name = %s" % class_descriptor_content
		...
		# # if config.debug:
			# # print "[GenericJniMethodEnd] method name = %s" % name_content
		...
		# # if config.debug:
			# # print "[GenericJniMethodEnd] return type = %s" % proto_return_type_descriptor_content
			...
			# for parameter_idx in range(len(proto_parameters_list)):
				...
				# else:
					...
					# else:
						# parameters_content = parameters_content + "," + parameter_type_descriptor_content
		# # if config.debug:
			# # print "[GenericJniMethodEnd] parameters = %s" % parameters_content
		
		# callee_method_signature = "%s->%s %s%s" % (class_descriptor_content, proto_return_type_descriptor_content, name_content, parameters_content)
		# if config.debug:
			# print "[GenericJniMethodEnd] method signature = %s" % callee_method_signature
	
	# config.log_print("[GenericJniMethodEnd] callee signature = %s" % callee_method_signature)
