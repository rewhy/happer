...
def InvokeWithArgArray():
	...
	method_ptr = int(execution_state.getRegisterService().getValue("R1")) & 0xffffffff
	if config.debug:
	...
	if config.debug:
		print "[InvokeWithArgArray] ArtMethod::access_flags_ = %#x (%s)" % (art_method_access_flags_value, config.resolve_access_flags(art_method_access_flags_value))
	...
	return_address = int(execution_state.getRegisterService().getValue("LR")) & 0xffffffff
	config.log_print("[InvokeWithArgArray] callee signature = %s, LR = %#x" % (callee_method_signature, return_address))
