...
def ArtInterpreterToInterpreterBridge():
	# get the "shadow_frame" parameter
	shadow_frame_ptr = int(execution_state.getRegisterService().getValue("R2"))
	if config.debug:
		print "[ArtInterpreterToInterpreterBridge] shadow_frame = %0#10x" % shadow_frame_ptr
	
	# -1- for caller
	
	# retrieve the "link_" field of ShadowFrame structure
	shadow_frame_link_ptr = memory.readMemory32(shadow_frame_ptr + config.offset_ShadowFrame_link_)
	
	caller_method_signature = None
	if memory.readMemory32(shadow_frame_link_ptr) != 0x0:
		# a previous shadow frame is existed
		
		# retrieve the "method_" field of ShadowFrame structure
		shadow_frame_method_ptr = memory.readMemory32(shadow_frame_link_ptr + config.offset_ShadowFrame_method_)
		...
		if config.debug:
			print "[ArtInterpreterToInterpreterBridge] ArtMethod::declaring_class_::dex_cache_::dex_file_::begin_ = %0#10x" % dex_file_begin_val
		...
		if config.debug:
			print "[ArtInterpreterToInterpreterBridge] ArtMethod::declaring_class_::dex_cache_::dex_file_::size_ = %#x" % dex_file_size_val
		...
		if config.debug:
			print "[ArtInterpreterToInterpreterBridge] ArtMethod::declaring_class_::dex_cache_::dex_file_::location_ = %s" % dex_file_location_string_val
		...
		if config.debug:
			print "[ArtInterpreterToInterpreterBridge] ArtMethod::access_flags_ = %#x (%s)" % (art_method_access_flags_value, config.resolve_access_flags(art_method_access_flags_value))
		...
		if config.debug:
			print "[ArtInterpreterToInterpreterBridge] ArtMethod::dex_code_item_offset_ = %#x" % art_method_dex_code_item_offset_value
		...
		if config.debug:
			print "[ArtInterpreterToInterpreterBridge] ArtMethod::dex_method_index_ = %#x" % art_method_dex_method_index_val
		...
		# if config.debug:
			# print "[ArtInterpreterToInterpreterBridge] class name = %s" % class_descriptor_content
		...
		# if config.debug:
			# print "[ArtInterpreterToInterpreterBridge] method name = %s" % name_content
		...
		# if config.debug:
			# print "[ArtInterpreterToInterpreterBridge] return type = %s" % proto_return_type_descriptor_content
			...
			for parameter_idx in range(len(proto_parameters_list)):
				...
				if len(proto_parameters_list) == 1:
					...
					if parameter_idx == 0:
						...
						parameters_content = parameters_content + "," + parameter_type_descriptor_content
		# if config.debug:
			# print "[ArtInterpreterToInterpreterBridge] parameters = %s" % parameters_content
		...
		# cannot find a previous shadow frame
		caller_method_signature = "unknown"
	
	if config.debug:
		print "[ArtInterpreterToInterpreterBridge] caller signature = %s" % caller_method_signature
	...
	if config.debug:
		print "[ArtInterpreterToInterpreterBridge] callee signature = %s" % callee_method_signature
	
	config.log_print("[ArtInterpreterToInterpreterBridge] caller signature = %s" % caller_method_signature)
	config.log_print("[ArtInterpreterToInterpreterBridge] callee signature = %s" % callee_method_signature)
