# ArtQuickToInterpreterBridge.py is used to ... when the execution flow enters Java code from native code in 32-bit mode.
...
def ArtQuickToInterpreterBridge():
	...
	if string_ids_off == 0x0 or type_ids_off == 0x0 or proto_ids_off == 0x0 or method_ids_off == 0x0:
		...
		# we only focus on the Compiled Code -> Java invocation appeared in the target package
		# however, we cannot disable DoCall breakpoint directly since it may miss some essential invocations
		# therefore, we define a framework method filter to wipe out noisy callee
		if config.framework_filter(callee_method_signature):
