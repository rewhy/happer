	...
	for vtl_start_address in sorted(mmu.page_table):
		...
		if (descriptor & 0x0040000000000000L) == 0x0000000000000000L and (descriptor & 0x0020000000000000L) == 0x0020000000000000L:
			if (descriptor & 0x00000000000000c0L) == 0x00000000000000c0L:
				...
				try:
					descriptor = descriptor & 0xffffffffffffff7f
				...
				try:
					...
					# if success, we have found a candidate, record it and jump out of this for-loop
					accessable_page_vtl_start_address = page_vtl_start_address
					accessable_page_phy_descriptor_address = page_phy_descriptor_address
					accessable_page_origin_descriptor = page_origin_descriptor
					break
				except DebugException:
					# if fail, we continue to search for the next candidate
					continue
	...
	while True:
		...
		try:
			...
			if brk_address == brk_force_loading_end_address:
				...
				print("unexpected breakpoint which stops at %0#10x" % brk_address)
