# Invoke.py is used to .... when the "ArtMethod::Invoke" method is invoked in 32-bit mode.
...
def Invoke():
	...
	method_ptr = int(execution_state.getRegisterService().getValue("R0")) & 0xffffffff
	if config.debug:
		print "[Invoke] callee = %0#10x" % method_ptr
	...
	if config.debug:
		print "[Invoke] ArtMethod::declaring_class_::dex_cache_::dex_file_::begin_ = %0#10x" % dex_file_begin_val
	...
	if config.debug:
		print "[Invoke] ArtMethod::declaring_class_::dex_cache_::dex_file_::size_ = %#x" % dex_file_size_val
	...
	if config.debug:
		print "[Invoke] ArtMethod::declaring_class_::dex_cache_::dex_file_::location_ = %s" % dex_file_location_string_val
	...
	if config.debug:
		print "[Invoke] ArtMethod::access_flags_ = %#x (%s)" % (art_method_access_flags_value, config.resolve_access_flags(art_method_access_flags_value))
	...
	if config.debug:
		print "[Invoke] ArtMethod::dex_code_item_offset_ = %#x" % art_method_dex_code_item_offset_value
	...
	if config.debug:
		print "[Invoke] ArtMethod::dex_method_index_ = %#x" % art_method_dex_method_index_val
	...
	if string_ids_off == 0x0 or type_ids_off == 0x0 or proto_ids_off == 0x0 or method_ids_off == 0x0:
		...
		# if config.debug:
			# print "[Invoke] class name = %s" % class_descriptor_content
		...
		# if config.debug:
			# print "[Invoke] method name = %s" % name_content
		...
		# if config.debug:
			# print "[Invoke] return type = %s" % proto_return_type_descriptor_content
			...
			for parameter_idx in range(len(proto_parameters_list)):
				...
				if len(proto_parameters_list) == 1:
					...
					if parameter_idx == 0:
						...
						parameters_content = parameters_content + "," + parameter_type_descriptor_content
		# if config.debug:
			# print "[Invoke] parameters = %s" % parameters_content
		...
		if config.debug:
			print "[Invoke] callee signature = %s" % callee_method_signature
		
		config.log_print("[Invoke] callee signature = %s" % callee_method_signature)
